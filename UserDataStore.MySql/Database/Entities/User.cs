...
internal class User
{
    ...
    public void Update(UserData data)
    {
        ...
        GenericDatas = data.Data?.Where(d => d.Value is not null).Select(d => new UserGenericData(d.Key, JsonSerializer.Serialize(d.Value), Id, Type)).ToList() ?? new List<UserGenericData>();
    }
